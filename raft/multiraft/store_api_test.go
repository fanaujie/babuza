...
func createStoreManager(storeConfigs []storeConfig, cuConfig customConfig, rootDir string,
	factory ComponentsFactory) (*StoreManager, error) {
	...
	for _, config := range storeConfigs {
		...
		if err = manager.Add(store); err != nil {
			return nil, err
		}
	}
	return manager, nil
...
func createStore(clusterID uint64, storeID uint64, cuConfig customConfig, storeRaftListenAddr string,
	...
	return BootstrapOrRecoverStore(config, factory, trans, walMgr, snapshotMgr, nil)
}

func proposeCommand(store *Store, groupID ibabuza.RaftGroupID, cmd CounterCommand) (*CounterResult, error) {
	cmdBytes, err := json.Marshal(cmd)
	if err != nil {
		return nil, err
	}

	res := store.Propose(context.Background(), groupID, babuza.ClientSession{}, cmdBytes)
...
func verifyCounterValue(manager *StoreManager, groupID ibabuza.RaftGroupID, expectedValue int64) error {
	c := int64(0)
	init := false

	for _, storeID := range manager.GetStoreIDsByGroupID(groupID) {
		store, err := manager.GetStore(storeID)
		if err != nil {
			return err
		}
		stateMachine, err := store.StateMachine(groupID)
		if err != nil {
			return err
		}
...
func TestBootstrap(t *testing.T) {
	...
	assert.Equal(t, 3, len(manager.GetAllStores()))
	...
	assert.NoError(t, store2.CreateRaftGroup(group2PeersConfig, false))
	assert.NoError(t, store3.CreateRaftGroup(group2PeersConfig, false))
	groupIDs := store1.GetGroupIDs()
	assert.NoError(t, err)
	assert.Equal(t, 1, len(groupIDs))
	assert.Equal(t, raftGroup1, groupIDs[0])
	groupIDs = store2.GetGroupIDs()
	assert.Equal(t, 2, len(groupIDs))
	assert.Equal(t, raftGroup1, groupIDs[0])
	assert.Equal(t, raftGroup2, groupIDs[1])
	groupIDs = store3.GetGroupIDs()
	assert.Equal(t, 1, len(groupIDs))
	...
	result, err := proposeCommand(group1LeaderStore, raftGroup1, CounterCommand{Operation: Reset, Value: 10})
	assert.NoError(t, err)
	assert.Equal(t, int64(10), result.Value)

	result, err = proposeCommand(group1LeaderStore, raftGroup1, CounterCommand{Operation: Increment, Value: 5})
	assert.NoError(t, err)
	...
	assert.NoError(t, err)
	result, err = proposeCommand(group2LeaderStore, raftGroup2, CounterCommand{Operation: Reset, Value: 20})
	assert.NoError(t, err)
	assert.Equal(t, int64(20), result.Value)
	result, err = proposeCommand(group2LeaderStore, raftGroup2, CounterCommand{Operation: Decrement, Value: 8})
	assert.NoError(t, err)
	assert.Equal(t, int64(12), result.Value)
	// wait for the command to be applied
	time.Sleep(time.Second)
	assert.NoError(t, verifyCounterValue(manager, raftGroup2, 12))
}
...
func TestRecover(t *testing.T) {
	...
	assert.NoError(t, peersConfig.AddPeer(3, 3, "localhost:14203", false))
	rootDir := t.TempDir()
	nm, err := createStoreManager(storeConfigs, defaultCustomConfig(), rootDir, newComponentFactory(NoOPSessionType))
	assert.NoError(t, err)
	assert.NotNil(t, nm)
	assert.Equal(t, 3, len(nm.GetAllStores()))
	for _, n := range nm.GetAllStores() {
		assert.NoError(t, n.Start())
	}
	store1, err := nm.GetStore(1)
	assert.NoError(t, err)
	store2, err := nm.GetStore(2)
	assert.NoError(t, err)
	store3, err := nm.GetStore(3)
	assert.NoError(t, err)
	defer func() {
		for _, n := range nm.GetAllStores() {
			n.Stop()
	...
	group1LeaderStore, err := nm.GetStore(group1LeaderID)
	assert.NoError(t, err)
	...
	group2LeaderStore, err := nm.GetStore(group2LeaderID)
	assert.NoError(t, err)
	result, err = proposeCommand(group2LeaderStore, raftGroup2, CounterCommand{Operation: Reset, Value: 20})
	assert.NoError(t, err)
	assert.Equal(t, int64(20), result.Value)
...
func TestJoinVotingGroup(t *testing.T) {
	...
	result, err := proposeCommand(group1LeaderStore, raftGroup1, CounterCommand{Operation: Reset, Value: 50})
	assert.NoError(t, err)
	assert.Equal(t, int64(50), result.Value)

	result, err = proposeCommand(group1LeaderStore, raftGroup1, CounterCommand{Operation: Increment, Value: 10})
	assert.NoError(t, err)
	...
	res := group1LeaderStore.AddVotingPeer(context.Background(), raftGroup1, babuza.ClientSession{}, peer3)
	ar := res.WaitForApplyResult()
	res.Release()
	assert.Nil(t, ar.Error)
	assert.NoError(t, group1PeersConfig.AddPeer(3, 3, "localhost:14203", false))
	assert.NoError(t, store3.CreateRaftGroup(group1PeersConfig, true))

	//wait for store3 to join group1 and apply the command
	time.Sleep(time.Second * 5)
	groupIDs = store3.GetGroupIDs()
	assert.Equal(t, 1, len(groupIDs))
	assert.Equal(t, raftGroup1, groupIDs[0])
...
func TestRemoveVotingGroup(t *testing.T) {
	...
	//remove group1
	res := group1LeaderStore.RemovePeer(context.Background(), raftGroup1, babuza.ClientSession{}, removeID)
	ar := res.WaitForApplyResult()
	res.Release()
	assert.Nil(t, ar.Error)
	...
	time.Sleep(time.Second)
	removeNode, err := nm.GetStore(removeID)
	assert.NoError(t, err)
	...
	time.Sleep(time.Second)
	storeIDs := nm.GetStoreIDsByGroupID(raftGroup1)
	assert.Equal(t, len(remainIDs), len(storeIDs))
	for index, storeID := range storeIDs {
		assert.Equal(t, remainIDs[index], storeID)
	}
...
func TestJoinLearnerAndPromoteLearner(t *testing.T) {
	...
	assert.NoError(t, peersConfig.AddPeer(1, 1, "localhost:14201", false))
	assert.NoError(t, peersConfig.AddPeer(2, 2, "localhost:14202", false))
	assert.NoError(t, peersConfig.AddPeer(3, 3, "localhost:14203", true)) // store3 設為 learner
	rootDir := t.TempDir()
	nm, err := createStoreManager(storeConfigs, defaultCustomConfig(), rootDir, newComponentFactory(NoOPSessionType))
	assert.NoError(t, err)
	assert.NotNil(t, nm)
	...
	result, err = proposeCommand(group1LeaderStore, raftGroup1, CounterCommand{Operation: Increment, Value: 7})
	assert.NoError(t, err)
	...
	peer3, _ := peersConfig.GetPeer(3)
	res := group1LeaderStore.AddLearner(context.Background(), raftGroup1, babuza.ClientSession{}, peer3)
	ar := res.WaitForApplyResult()
	res.Release()
	assert.Nil(t, ar.Error)
	assert.NoError(t, group1PeersConfig.AddPeer(3, 3, "localhost:14203", true))
	assert.NoError(t, store3.CreateRaftGroup(group1PeersConfig, true))
	...
	cfg, _ := group1LeaderStore.RaftGroupPeersInfo(groupIDs[0])
	for _, peer := range cfg.Peers {
		if peer.PeerID == peer3.PeerID {
			assert.True(t, peer.IsLearner)
		}
	}

	res = group1LeaderStore.PromoteLearner(context.Background(), raftGroup1, babuza.ClientSession{}, peer3.PeerID)
	ar = res.WaitForApplyResult()
	res.Release()
	assert.Nil(t, ar.Error)
...
func TestTransferLeader(t *testing.T) {
	...
	group2PeersConfig.SetGroupID(raftGroup2)
	assert.NoError(t, store1.CreateRaftGroup(group1PeersConfig, false))
	assert.NoError(t, store1.CreateRaftGroup(group2PeersConfig, false))
	assert.NoError(t, store2.CreateRaftGroup(group1PeersConfig, false))
	assert.NoError(t, store2.CreateRaftGroup(group2PeersConfig, false))
	assert.NoError(t, store3.CreateRaftGroup(group1PeersConfig, false))
	assert.NoError(t, store3.CreateRaftGroup(group2PeersConfig, false))
	...
	result1 := store1.TransferLeader(context.Background(), raftGroup1, group1Transferee)
	result2 := store2.TransferLeader(context.Background(), raftGroup2, group2Transferee)
...
func TestSnapshot(t *testing.T) {
	...
	nm, err := createStoreManager(storeConfigs, cuConfig, rootDir, newComponentFactory(NoOPSessionType))
	assert.NoError(t, err)
	assert.NotNil(t, nm)
	assert.Equal(t, 3, len(nm.GetAllStores()))
	store1, err := nm.GetStore(1)
	...
	assert.NoError(t, err)
	assert.NoError(t, store3.Start())
	assert.NoError(t, nm.Add(store3))
...
func TestMultipleGroup(t *testing.T) {
	...
	for i := 0; i < totalGroups; i++ {
		groupID := ibabuza.RaftGroupID(i + 1)
		leaderStore, err := nm.GetStore(leaderIDs[groupID])
		assert.NoError(t, err)
		wg.Add(1)
		go func(store *Store, gid ibabuza.RaftGroupID) {
			defer wg.Done()
			result, err := proposeCommand(store, gid, CounterCommand{Operation: Reset, Value: 0})
			assert.NoError(t, err)
			assert.Equal(t, int64(0), result.Value)
			for j := 0; j < proposeCount; j++ {
				r, pErr := proposeCommand(store, gid, CounterCommand{Operation: Increment, Value: 1})
...
func TestRegisterSession(t *testing.T) {
	...
	nm, err := createStoreManager(storeConfigs, defaultCustomConfig(), rootDir, newComponentFactory(LruSessionType))
	assert.NoError(t, err)
	assert.NotNil(t, nm)
	...
	leaderStore, err := nm.GetStore(leaderID)
	assert.NoError(t, err)

	ctx := context.Background()
	res := leaderStore.RegisterSession(ctx, raftGroup)
	ar := res.WaitForApplyResult()
	...
	res = leaderStore.Propose(ctx, raftGroup, session1, cmdBytes1)
	ar = res.WaitForApplyResult()
	assert.Nil(t, ar.Error)
	...
	res = leaderStore.Propose(ctx, raftGroup, session1, cmdBytes2)
	ar = res.WaitForApplyResult()
	res.Release()
	assert.Nil(t, ar.Error)
	...
	res = leaderStore.Propose(ctx, raftGroup, session2, cmdBytes3)
	ar = res.WaitForApplyResult()
	res.Release()
	assert.Nil(t, ar.Error)
	...
	// unregister session
	res = leaderStore.UnregisterSession(ctx, raftGroup, sessionID1)
	ar = res.WaitForApplyResult()
	assert.Nil(t, ar.Error)
	...
	assert.NoError(t, err)
	res = leaderStore.Propose(ctx, raftGroup, session1, cmdBytes4)
	ar = res.WaitForApplyResult()
...
func TestLinearizableRead(t *testing.T) {
	...
	for i := 0; i < totalGroups; i++ {
		...
		leaderStore, err := nm.GetStore(leaderIDs[groupID])
		assert.NoError(t, err)
		assert.Nil(t, leaderStore.LinearizableRead(context.Background(), groupID))
		fsm, err := leaderStore.StateMachine(groupID)
		assert.NoError(t, err)
		assert.Equal(t, int64(proposeCount), fsm.(*SimpleStateMachine).Counter())
	}
}

func TestCreateRawReplica(t *testing.T) {
	storeConfigs := []storeConfig{
		{storeID: 1, raftListenAddr: "localhost:14201"},
		{storeID: 2, raftListenAddr: "localhost:14202"},
		{storeID: 3, raftListenAddr: "localhost:14203"},
	...
	// create raw replica as a learner on store3
	peer3, _ := peersConfig.GetPeer(3)
	peer3.IsLearner = true
	leaderPeer, _ := group1PeersConfig.GetPeer(group1LeaderID)
	assert.NoError(t, store3.CreateBasicRaftGroup(raftGroup1, peer3.PeerID, leaderPeer.PeerID, leaderPeer.RaftListenAddr))
	groupIDs = store3.GetGroupIDs()
	assert.Equal(t, 1, len(groupIDs))
	assert.Equal(t, raftGroup1, groupIDs[0])

	// leader should be able to add learner to the group
	res := group1LeaderStore.AddLearner(context.Background(), raftGroup1, babuza.ClientSession{}, peer3)
	ar := res.WaitForApplyResult()
	res.Release()
	assert.NoError(t, ar.Error)

	// wait for store3 to join as a learner
	time.Sleep(time.Second)
	assert.NoError(t, verifyCounterValue(manager, raftGroup1, 15))
}
