...
type Node struct {
	...
	scheduler     ibabuza.Scheduler
	applyJobQueue ibabuza.ApplyJobQueue
	log           ibabuza.Logger
	multiStatus   ibabuza.MultiStatus
	replicaSet    struct {
		mu      sync.RWMutex
...
func StartNode(config NodeConfig, trans ibabuza.Transport, log ibabuza.Logger) *Node {
...
func RestartNode(config NodeConfig, trans ibabuza.Transport) *Node {
	return &Node{}
}
...
func (n *Node) Propose(ctx context.Context, gid ibabuza.RaftGroupID, session raft.ClientSession, log []byte) raft.ProposedResult {
	replica, ok := n.replicaSet.replica[gid]
	if !ok {
		return raft.NewErrorResult(errors.Errorf("raft group %d not found", gid))
	}
	if err := n.scheduler.EnqueueState(gid, shard.StateProposal); err != nil {
		return raft.NewErrorResult(err)
	}
	return replica.EnqueueProposal(ctx, session, log)
}

func (n *Node) AddVotingPeer(ctx context.Context, gid ibabuza.RaftGroupID, session raft.ClientSession,
	raftPeerAttr babuzapb.RaftPeerAttribute) raft.ProposedResult {
	replica, ok := n.replicaSet.replica[gid]
	if !ok {
		return raft.NewErrorResult(errors.Errorf("raft group %d not found", gid))
	}
	if n.config.DisableProposalForwarding && n.multiStatus.Get(gid).IsLeader() == false {
	...
	if err := n.scheduler.EnqueueState(gid, shard.StateConfigChange); err != nil {
		return raft.NewErrorResult(err)
	}
