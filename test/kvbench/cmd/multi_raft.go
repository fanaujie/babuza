...
func init() {
    ...
    multiCmd.Flags().StringVar(&initialPeersRaw, "initial-peers", "", "List of initial peers to connect to when joining (e.g., 1=127.0.0.1:14200,2=127.0.0.1:14201)")

    // Add the shard count flag specific to multi command
...
func runMultiServerFunc(cmd *cobra.Command, args []string) {
    ...
    fmt.Printf("Starting multi-raft server with %d shards, gRPC on %s and Raft on %s...\n",
        shardCount, cfg.GrpcAddress, cfg.RaftAddress)
